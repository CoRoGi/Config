...
augroup BlendCursor
  autocmd!
  autocmd Filetype * hi Cursor blend=50
augroup END
]]

vim.cmd [[
augroup SetFiletypeMD
  autocmd!
...
-- Define the directory where the diary files should be created
local diary_directory = "/Users/crg/Personal/Obsidian/diary/"

-- Function to check if the file path is within the diary directory
function is_diary_file()
  local file_path = vim.fn.expand "%:p"

  if string.find(file_path, diary_directory, 1, true) == 1 then
    local file_name = vim.fn.expand "%:t"
    local pattern = "^%d%d%d%d%-%d%d%-%d%d.md"
    return string.match(file_name, pattern) ~= nil
  end

  return false
end

-- Autocommand to trigger when a new file is created
vim.cmd [[
augroup DiaryTemplate
  autocmd!
  autocmd BufNewFile * if luaeval('is_diary_file()') | call setline(1, "# " .. strftime("%Y-%m-%d")) | endif
augroup END
]]
...
vim.cmd [[
  augroup CalendarCredentials
  autocmd!
  autocmd VimEnter * ++nested ++once source ~/.cache/calendar.vim/credentials.vim
  augroup END
]]
...
autocmd({ "BufEnter" }, {
  group = augroup("neotree_start", { clear = true }),
  callback = function() vim.cmd "highlight NeoTreeNormal guibg=None" end,
})

autocmd({ "VimEnter" }, {
  group = augroup("Neovim_startup", { clear = true }),
  callback = function() vim.cmd "highlight TelescopeNormal guibg=None" end,
})
